...
<div>
	...
	<div class="news-preview-pv" :style='{"minHeight":"660px","padding":"10px calc((100% - 1200px)/2) 0","margin":"0px auto","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0","margin":"0 0 20px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"flex-end","height":"auto"}'>
			<el-form-item :style='{"margin":"0 0px"}'>
				<el-input v-model="title" placeholder="标题"></el-input>
			</el-form-item>
			<el-button type="primary" @click="getNewsList(1)" :style='{"cursor":"pointer","border":"0","padding":"0px 20px 0 10px","margin":"0 0px 0 10px","outline":"none","color":"#fff","borderRadius":"0 0px 0px 0","background":"rgba(30,124,9,1)","width":"80px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>
		...
		<!-- category -->
		<div class="category" :style='{"padding":"130px 0px 20px","margin":"0px 20px 0 0","borderColor":"#ddd","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240201/fc8536ae3aec4e2992eb717a4d3448e1.png) no-repeat center top,url(http://codegen.caihongy.cn/20240201/309d30a1a11d43608645dec0455ea227.png) no-repeat center bottom,url(http://codegen.caihongy.cn/20240201/3a23948b134c4a6db28c8b5b71e97fde.png) repeat-y center center","borderWidth":"0px 0","display":"flex","width":"260px","borderStyle":"solid","justifyContent":"flex-start","height":"auto","order":"2"}'>
		  ...
		  <div v-for="(item,index) in categoryList" @click="categoryClick(index+1)" :key="index" class="item" :class="categoryIndex == index+1 ? 'active' : ''">{{item.typename}}</div>
				...
				<!-- 样式五 -->
		<div class="list5 index-pv1" :style='{"border":"1px solid rgb(123, 123, 123)","padding":"20px 30px","margin":"20px 0 0","color":"#a6a6a6","display":"block","minHeight":"500px","borderRadius":"10px","flexWrap":"wrap","background":"#fff","flex":"1","width":"calc(100% - 280px)","fontSize":"12px","height":"auto","order":"3"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 10px","margin":"0 0 10px","borderColor":"#ccc","background":"#fff","borderWidth":"0 0 2px","display":"flex","width":"100%","borderStyle":"dotted","height":"auto"}' v-for="item in newsList" :key="item.id" @click="toNewsDetail(item)" class="list-item animation-box">
				<el-image :style='{"border":"0px solid #ddd","padding":"0px","margin":"0 20px 0 0","objectFit":"cover","display":"none","width":"160px","height":"120px"}' :src="baseUrl + item.picture" :fit="fill"></el-image>
				<div class="item-info" :style='{"padding":"0px","alignItems":"flex-start","flexWrap":"wrap","display":"flex","width":"calc(100% - 0px)","position":"relative","height":"auto"}'>
					<div :style='{"overflow":"hidden","whiteSpace":"nowrap","color":"#5a5a5a","width":"100%","lineHeight":"24px","fontSize":"14px","textOverflow":"ellipsis","fontWeight":"600"}' class="title ">{{item.title}}</div>
					<div :style='{"width":"100%","lineHeight":"24px","fontSize":"12px","color":"#666","order":"2"}' class="introduction ">{{item.introduction}}</div>
					<div :style='{"width":"auto","padding":"5px 0","margin":"0 10px 0 0","display":"inline-block"}'>
		...
		<!-- 热门信息 -->
		<div class="hot" :style='{"border":"0px solid #e9e9e9","padding":"0","margin":"20px 0 0","background":"none","width":"100%","clear":"both","height":"auto","order":"10"}'>
		  <div :style='{"padding":"0px 20px","borderColor":"#fb3f52","color":"rgba(22,159, 25,1)","textAlign":"center","display":"inline-block","borderRadius":"0px","background":"url(http://codegen.caihongy.cn/20240131/2a430ef16f4e4cd097649b7ed3473f46.png) no-repeat center bottom","borderWidth":"0 0 0px","width":"100%","lineHeight":"44px","fontSize":"22px","borderStyle":"solid","fontWeight":"500"}'>推荐信息</div>
		  <div :style='{"padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"100%","padding":"0","margin":"0 0 0px","background":"#fff","height":"auto"}' @click="toNewsDetail(item)">
		      ...
		      <div :style='{"padding":"0 10px","borderColor":"#ddd","overflow":"hidden","whiteSpace":"nowrap","color":"#666","textAlign":"left","borderWidth":"0 0 2px","width":"100%","lineHeight":"50px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dotted"}'>{{ item.title }}</div>
